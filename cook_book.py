...
#  функция для получения списка покупок, исходя из заказанных блюд и количества персон
def get_shop_list_by_dishes(dishes, person_count):
    ...
    for _dish in dishes:
        for ingredient in cook_book[_dish]:  # Выбираем ингредиенты для блюд из поваренной книги
            ...
            #  Проверяем заказанное блюдо на предмет повтора
...
print(get_shop_list_by_dishes(['Запеченный картофель', 'Омлет', 'Фахитос', 'Омлет'],13))